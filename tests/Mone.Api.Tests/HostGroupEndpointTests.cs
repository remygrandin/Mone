...
public class HostGroupEndpointTests
{
    ...
    private async Task<(HttpClient Client, Guid HostId)> CreateClientAndHostAsync()
    {
        var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);
    ...
    private async Task<HostGroupResponse> CreateGroupAsync(HttpClient client, string? name = null, Guid? parentId = null)
    {
        ...
        var group = await resp.Content.ReadFromJsonAsync<HostGroupResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CreateGroup_ValidData_Returns201()
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_create_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);
    ...
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_nested_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);
    ...
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_cycle_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);
        ...
        var resp = await client.PutAsJsonAsync($"/api/host-groups/{a.Id}",
            new UpdateHostGroupRequest($"A-{uid}", null, c.Id), cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_self_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);

        var name = $"A-{Guid.NewGuid():N}";
        var a = await CreateGroupAsync(client, name);

        var resp = await client.PutAsJsonAsync($"/api/host-groups/{a.Id}",
            new UpdateHostGroupRequest(name, null, a.Id), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AddMember_VerifyMembership()
    {
        ...
        var getResp = await client.GetAsync($"/api/host-groups/{group.Id}", TestContext.Current.CancellationToken);
        var detail = await getResp.Content.ReadFromJsonAsync<HostGroupDetailResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_del_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);

        var group = await CreateGroupAsync(client);

        var resp = await client.DeleteAsync($"/api/host-groups/{group.Id}", TestContext.Current.CancellationToken);
    ...
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_delc_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);

        var parent = await CreateGroupAsync(client, "Parent");
        await CreateGroupAsync(client, "Child", parent.Id);

        var resp = await client.DeleteAsync($"/api/host-groups/{parent.Id}", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.BadRequest, resp.StatusCode);
    }
    ...
    {
        using var client = await _fixture.CreateAuthenticatedClientAsync(
            $"hg_upd_{Guid.NewGuid():N}@test.com", "ValidPass1!", TestContext.Current.CancellationToken);

        var group = await CreateGroupAsync(client, "Original");

        var resp = await client.PutAsJsonAsync($"/api/host-groups/{group.Id}",
            new UpdateHostGroupRequest("Updated", "A description", null), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);

        var updated = await resp.Content.ReadFromJsonAsync<HostGroupResponse>(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ListGroups_ReturnsCounts()
    {
        ...
        var resp = await client.GetAsync("/api/host-groups", TestContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);

        var groups = await resp.Content.ReadFromJsonAsync<HostGroupResponse[]>(cancellationToken: TestContext.Current.CancellationToken);
