...
public class OperatorJourneyTests
{
    ...
    public async Task OperatorJourney_LoginToOverride_TraversesAsOneProduct()
    {
        ...
        var listResp = await client.GetAsync("/api/hosts");
        ...
        var createResp = await client.PostAsJsonAsync("/api/hosts",
            new CreateHostRequest(hostName, "10.0.0.1"));
        Assert.Equal(HttpStatusCode.Created, createResp.StatusCode);
        var host = await createResp.Content.ReadFromJsonAsync<HostResponse>();
        Assert.NotNull(host);
        ...
        var probesResp = await client.GetAsync($"/api/hosts/{hostId}/probes");
        Assert.Equal(HttpStatusCode.OK, probesResp.StatusCode);
        var seededProbes = await probesResp.Content.ReadFromJsonAsync<ProbeAssignmentResponse[]>();
        Assert.NotNull(seededProbes);
        Assert.Empty(seededProbes);

        var statusResp = await client.GetAsync($"/api/hosts/{hostId}/status/latest");
        ...
        var directAssignResp = await client.PostAsJsonAsync($"/api/hosts/{hostId}/probes",
            new CreateProbeAssignmentRequest("ping", $"ping-{suffix}", "*/5 * * * *",
                ConfigJson: """{"timeout":30,"retries":3}"""));
        Assert.Equal(HttpStatusCode.Created, directAssignResp.StatusCode);
        var directAssignment = await directAssignResp.Content.ReadFromJsonAsync<ProbeAssignmentResponse>();
        ...
        var groupResp = await client.PostAsJsonAsync("/api/host-groups",
            new CreateHostGroupRequest($"journey-grp-{suffix}", null, null));
        Assert.Equal(HttpStatusCode.Created, groupResp.StatusCode);
        var hostGroup = await groupResp.Content.ReadFromJsonAsync<HostGroupResponse>();
        ...
        var groupAssignResp = await client.PostAsJsonAsync($"/api/host-groups/{groupId}/probes",
            new CreateProbeAssignmentRequest("http", $"http-{suffix}", "*/10 * * * *",
                ConfigJson: """{"timeout":15}"""));
        Assert.Equal(HttpStatusCode.Created, groupAssignResp.StatusCode);
        var groupAssignment = await groupAssignResp.Content.ReadFromJsonAsync<ProbeAssignmentResponse>();
        ...
        var overrideResp = await client.PutAsJsonAsync(
            ...
            new UpsertOverrideRequest { ConfigJsonOverride = """{"timeout":45}""", IsDisabled = false });
        ...
        var badResp = await client.PostAsJsonAsync("/api/hosts",
            new CreateHostRequest("", "10.0.0.2"));
        await AssertProblemDetailsAsync(badResp, HttpStatusCode.BadRequest);
    }
}
