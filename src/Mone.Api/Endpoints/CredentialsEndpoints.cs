...
{
    public record CredentialsResponse(Guid Id, string Name, string Username, string Password, DateTimeOffset CreatedAt, DateTimeOffset UpdatedAt);
    ...
    private static async Task<IResult> CreateCredentials(CreateCredentialsRequest request, MoneDbContext db)
    {
        ...
        return Results.Created($"/api/credentials/{credentials.Id}",
            new CredentialsResponse(credentials.Id, credentials.Name, credentials.Username, credentials.Password, credentials.CreatedAt, credentials.UpdatedAt));
    }
    ...
    private static async Task<IResult> GetCredentials(Guid id, MoneDbContext db)
    {
        ...
        return Results.Ok(new CredentialsResponse(credentials.Id, credentials.Name, credentials.Username, credentials.Password, credentials.CreatedAt, credentials.UpdatedAt));
    }
